...
void DbusToFileHandler::sendNewFileAvailableCmd(uint64_t fileSize)
{
    ...
    char* end;
    uint32_t fileHandle = std::strtoul(filename.c_str(), &end, 16);

    info(
        "File handle in sendNewFileAvailableCmd that we encode new_file_req is {FILEHANDLE}",
        "FILEHANDLE", filename);
    ...
    const std::string& vspString, const std::string& resDumpReqPass)
{
    ...
    info("In processNewResourceDump fileHandle is {FILEHANDLE}", "FILEHANDLE",
         filename);
...
void DbusToFileHandler::newLicFileAvailable(const std::string& licenseStr)
{
    ...
    if (!licFile)
    {
        error("Failed to open license file {LIC_PATH}", "LIC_PATH",
              licFilePath);
        return;
    }
