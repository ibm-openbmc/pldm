...
{
    info(
        "Remove this once CI compiles and AIO changes are there: length = '{LEN}' and address {ADDR}",
        "LEN", length, "ADDR", address);
    /* if (DumpHandler::fd == -1)
     {
         ...
         {
             error(
                 "DumpHandler::writeFromMemory: transferFileDataToSocket
     failed"); if (DumpHandler::fd >= 0)
             {
                 close(DumpHandler::fd);
                 DumpHandler::fd = -1;
             }
                                              ...
                                              uint32_t /*metaDataValue4*/)
{
    ...
    try
    {
        ...
        auto method = bus.new_method_call(
            ...
            "NotifyDump"); // need to change the method to "NotifyWithToken"
                           // once dump manager changes are merged
        method.append(fileHandle, length, notifyDumpType,
                      metaDataValue1); // need to append metaDataValue1 once
