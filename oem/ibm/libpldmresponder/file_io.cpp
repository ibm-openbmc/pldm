                          ...
                          uint64_t address, bool upstream)
{
    ...
    {
        rc = -errno;
        std::cerr << "Failed to open the XDMA device, RC=" << rc << std::endl;
        return rc;
    }
    ...
    {
        rc = -errno;
        std::cerr << "Failed to mmap the XDMA device, RC=" << rc << std::endl;
        return rc;
    }
                                     ...
                                     size_t payloadLength)
{
    ...
    {
        std::cerr << "File does not exist, HANDLE=" << fileHandle << std::endl;
        encode_rw_file_memory_resp(request->hdr.instance_id,
                                   PLDM_READ_FILE_INTO_MEMORY,
                                   PLDM_INVALID_FILE_HANDLE, 0, responsePtr);
        return response;
...
Response Handler::fileAck(const pldm_msg* request, size_t payloadLength)
{
    ...
    auto rc =
        decode_file_ack_req(request, payloadLength, &fileType, &fileHandle, &fileStatus);
    if (rc != PLDM_SUCCESS)
    {
