...
void SlotHandler::timeOutHandler()
{
    ...
    sendStateSensorEvent(sensorId, PLDM_STATE_SENSOR_STATE, 0,
                         uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_ERROR),
                         uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_UNKOWN));
}
                                 ...
                                 uint8_t stateFiledValue)
{
    ...
    try
    {
        auto service = pldm::utils::DBusHandler().getService(VPDObjPath,
                                                             VPDInterface);
        if (stateFiledValue == uint8_t(PLDM_OEM_IBM_PCIE_SLOT_EFFECTER_ADD))
        {
        ...
        else if (stateFiledValue ==
                     uint8_t(PLDM_OEM_IBM_PCIE_SLOT_EFFECTER_REMOVE) ||
                 stateFiledValue ==
                     uint8_t(PLDM_OEM_IBM_PCIE_SLOT_EFFECTER_REPLACE))
        {
            auto method = bus.new_method_call(service.c_str(), VPDObjPath,
    ...
    catch (const std::exception& e)
    {
        error(
            ...
            "STATE_FILED_VAL", (unsigned)stateFiledValue, "ERROR", e);
    }
}
    ...
    uint8_t stateFiledvalue, const pldm_entity& entity)
{
    ...
    uint8_t sensorOpState = uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_UNKOWN);
    if (presentValue)
    {
        sensorOpState = uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_ENABLED);
    }
    else
    {
        if (stateFiledvalue == uint8_t(PLDM_OEM_IBM_PCIE_SLOT_EFFECTER_REPLACE))
        {
            sensorOpState = uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_UNKOWN);
        }
        else
        {
            sensorOpState =
                uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_DISABLED);
        }
    }
    info(
...
uint8_t SlotHandler::fetchSlotSensorState(const std::string& slotObjectPath)
{
    ...
    catch (const std::bad_optional_access& e)
    {
        ...
        return uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_UNKOWN);
    }
    ...
    {
        sensorOpState = uint8_t(PLDM_OEM_IBM_PCIE_SLOT_SENSOR_STATE_DISABLED);
    }
...
bool SlotHandler::fetchSensorStateFromDbus(const std::string& adapterObjectPath)
{
    ...
    {
        error("failed to make a d-bus call to Inventory manager, ERROR={ERROR}",
              "ERROR", e);
    }
