                     ...
                     std::vector<uint8_t>& responseMsg, bool pldmVerbose)
{
    ...
    {
        returnCode = -errno;
        lg2::error("Failed to connect to socket : RC = {KEY0}", "KEY0", returnCode);
    ...
    if (0 == peekedLength)
    {
        lg2::error("Socket is closed : peekedLength = {KEY0}", "KEY0", peekedLength);
    ...
    else
    {
        ...
        do
        {
            ...
            {
                lg2::error("Failure to read response length packet: length = {KEY0}", "KEY0", recvDataLength);
                return returnCode;
            }
        } while (1);
    }
    ...
    {
        returnCode = -errno;
        lg2::error("Failed to shutdown the socket : RC ={KEY0}", "KEY0", returnCode);
        return returnCode;
    }
...
void CommandInterface::exec()
{
    ...
    {
        lg2::error("GetInstanceId D-Bus call failed, MCTP id = {KEY0}, error = {KEY1}", "KEY0", (unsigned)mctp_eid, "KEY1", e.what());
        return;
    }
    auto [rc, requestMsg] = createRequestMsg();
    if (rc != PLDM_SUCCESS)
    {
        lg2::error("Failed to encode request message for {KEY0} : {KEY1}, rc = {KEY1}", "KEY0", pldmType, "KEY1", commandName, "KEY1", rc);
        return;
    }
