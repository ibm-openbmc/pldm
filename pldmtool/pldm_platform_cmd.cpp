    ...
    void exec() override
    {
        if (allPDRs || !pdrRecType.empty())
        {
            ...
            do
            {
                ...
                if (!result.second)
                {
                    lg2::error("Record handle {KEY0} has multiple references: {KEY1}, {KEY2}", "KEY0",recordHandle, "KEY1", result.first->second, "KEY2", prevRecordHandle);
                    return;
                }
  ...
  private:
                     ...
                     uint8_t* data)
    {
        ...
        if (!pdrRecType.empty())
        {
            ...
            {
                lg2::error( "PDR type {KEY0} is not supported or invalid", "KEY0", pdrRecType);
...
class SetStateEffecter : public CommandInterface
{
  public:
    ...
    std::pair<int, std::vector<uint8_t>> createRequestMsg() override
    {
        ...
        {
            lg2::error( "Request Message Error: effecterCount size {KEY0} is invalid", "KEY0",effecterCount);
        ...
        {
            lg2::error( "Request Message Error: effecterData size {KEY0} is invalid", "KEY0", effecterData.size());
            auto rc = PLDM_ERROR_INVALID_DATA;
            return {rc, requestMsg};
        }
