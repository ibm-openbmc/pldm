...
void HostAssociationsParser::parseHostAssociations(const std::string& jsonPath)
{
    ...
    for (const auto& entry : entries)
    {
        ...
        bool parentChild_assocation = entry.value("parent_child_association", false);
        associationsInfoMap[std::make_pair(from_entity_type, to_entity_type)] =
            std::make_tuple(parentChild_assocation, std::make_pair(forward_association, reverse_assocation));
    }
}
