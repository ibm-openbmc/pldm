...
void HostPDRHandler::_setHostSensorState()
{
    ...
    if (!pdr)
    {
        ...
        pldm::utils::reportError(
            "xyz.openbmc_project.bmc.pldm.GetStateSensorPDRFail");
        return;
    }
        ...
        if (terminusHandle == pdr->terminus_handle)
        {
            ...
            if (rc != PLDM_SUCCESS)
            {
                ...
                std::cerr << "Failed to "
                          ...
                          << rc << std::endl;
                pldm::utils::reportError(
                    "xyz.openbmc_project.bmc.pldm.EncodeStateSensorFail");
                return;
            }
            ...
            auto getStateSensorReadingRespHandler = [=,
                ...
                if (rc != PLDM_SUCCESS || completionCode != PLDM_SUCCESS)
                {
                    std::cerr << "Failed to "
                              ...
                              << std::endl;
                    pldm::utils::reportError(
                        "xyz.openbmc_project.bmc.pldm.DecodeStateSensorFail");
                }
