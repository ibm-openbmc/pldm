                     ...
                     (leftPath.string().find(rightPath1) != std::string::npos))
            {
                ...
                if (associationsParser->associationsInfoMap.contains(key))
                {
                    ...
                    if (std::get<0>(value))
                    {
                        ...
                        {
                            std::cout << "Not a parent child relation " << rightPath.string() << "\n";
                            std::cout << " left child " << leftPath.string();
                            return;
                        }
                    }
                    // we have some associations defined for this ancenstor and child type
                    // leftpath = /xyz/openbmc_project/system/chassis15363
                    // rightpath = /xyz/openbmc_project/system/chassis15363/Slot1/adapter1

                    std::vector<
                        std::tuple<std::string, std::string, std::string>>
                        associations{{(std::get<1>(value)).first, (std::get<1>(value)).second, rightPath}};
