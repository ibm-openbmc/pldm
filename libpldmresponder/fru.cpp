                                    ...
                                    const std::string& path)
{
    ...
    for (; i >= 0; i--)
    {
        if (objToEntityNode.contains(tmpObjPaths[i]))
        {
            ...
            if (pldm_entity_association_tree_find_with_locality(entityTree, &node_entity,
                                                  false))
            {
                continue;
            }
        ...
        for (const auto& interface : interfaces)
        {
            if (itemIntfsLookup.contains(interface.first))
            {
                ...
                try
                {
                    ...
                    {
                        entity = objToEntityNode.at(object.first.str);
                    }
    ...
    const dbus::ObjectPath& objectPath, bool concurrentAdd)
{
    ...
    for (const auto& [recType, encType, fieldInfos] : recordInfos)
    {
        ...
        if (tlvs.size())
        {
		    ...
		    bmc_record_handle = pldm_pdr_get_record_handle(pdrRepo, lastLocalRecord) + 1;
#endif
                }
                else
                {
...
void FruImpl::removeIndividualFRU(const std::string& fruObjPath)
{
    ...
    pldm_pdr_remove_fru_record_set_by_rsi(pdrRepo, rsi, false, &deleteRecordHdl);
                                 ...
                                 const std::string& fruObjectPath)
{
...
#ifdef OEM_IBM
    auto lastLocalRecord = pldm_pdr_find_last_in_range(pdrRepo, BMC_PDR_START_RANGE, BMC_PDR_END_RANGE);
    bmc_record_handle = pldm_pdr_get_record_handle(pdrRepo, lastLocalRecord);
#endif

    uint8_t bmcEventDataOps;
    uint32_t updatedRecordHdlBmc = 0;
    bool found = false;
    pldm_entity_association_find_parent_entity(pdrRepo, &parentEntity, false,
...
#ifdef OEM_IBM
    auto lastBMCRecord = pldm_pdr_find_last_in_range(pdrRepo, HOST_PDR_START_RANGE, HOST_PDR_END_RANGE);
    ...
    found = false;
    pldm_entity_association_find_parent_entity(pdrRepo, &parentEntity, true,
